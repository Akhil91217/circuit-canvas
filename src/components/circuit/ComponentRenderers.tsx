...
export function LEDRenderer({ def, properties, pinStates }: RendererProps) {
  ...
  return (
    <Group>
      <Circle x={def.width / 2} y={18} radius={12} fill={fill} opacity={glowOpacity} shadowColor={fill} shadowBlur={isOn ? 20 : 4} shadowOpacity={isOn ? 0.9 : 0.2} />
...
export function OLEDRenderer({ def, properties }: RendererProps) {
  ...
  return (
    <Group>
      ...
      {text && <Text x={10} y={10} text={text.slice(0, 20)} fontSize={8} fill="#00d4ff" fontFamily="monospace" width={def.width - 20} />}
...
export function ServoRenderer({ def, properties }: RendererProps) {
  ...
  return (
    <Group>
      ...
      <Line points={[def.width / 2, def.height / 2, def.width / 2 + Math.cos((angle - 90) * Math.PI / 180) * 16, def.height / 2 + Math.sin((angle - 90) * Math.PI / 180) * 16]} stroke="#ccc" strokeWidth={3} lineCap="round" />
      <Circle x={def.width / 2 + Math.cos((angle - 90) * Math.PI / 180) * 16} y={def.height / 2 + Math.sin((angle - 90) * Math.PI / 180) * 16} radius={3} fill="#ddd" />
...
export function BME280Renderer({ def }: RendererProps) {
  return (
    <Group>
      ...
      <Rect x={6} y={4} width={24} height={12} fill="#1a1a2e" cornerRadius={1} stroke="#333" strokeWidth={0.5} />
      <Circle x={18} y={10} radius={3} fill="#333" stroke="#555" strokeWidth={0.3} />
      <Text x={4} y={20} text="BME" fontSize={7} fill="#5eead4" fontFamily="monospace" fontStyle="bold" />
...
export function PIRRenderer({ def, properties }: RendererProps) {
  ...
  return (
    <Group>
      ...
      <Circle x={def.width / 2} y={16} radius={14} fill="#292524" stroke={detected ? '#ef4444' : '#57534e'} strokeWidth={1.5} />
      <Circle x={def.width / 2} y={16} radius={8} fill={detected ? '#dc262680' : '#44403c'} shadowColor={detected ? '#ef4444' : 'transparent'} shadowBlur={detected ? 10 : 0} />
      <Text x={8} y={34} text="PIR" fontSize={7} fill="#a8a29e" fontFamily="monospace" fontStyle="bold" />
    </Group>
  );
}
...
export function GPSRenderer({ def }: RendererProps) {
  return (
    <Group>
      ...
      <Rect x={12} y={4} width={26} height={20} fill="#1a1a2e" cornerRadius={2} stroke="#333" strokeWidth={0.5} />
      <Text x={16} y={8} text="GPS" fontSize={7} fill="#4ade80" fontFamily="monospace" fontStyle="bold" />
      <Circle x={25} y={36} radius={6} fill="#166534" stroke="#22c55e" strokeWidth={0.5} />
      <Text x={22} y={33} text="🛰️" fontSize={6} />
    </Group>
  );
}

export function TFTRenderer({ def, properties }: RendererProps) {
  return (
    <Group>
      ...
      {/* Color bars to indicate TFT */}
      <Rect x={10} y={10} width={def.width - 24} height={3} fill="#ef4444" cornerRadius={1} />
      <Rect x={10} y={16} width={def.width - 24} height={3} fill="#22c55e" cornerRadius={1} />
      <Rect x={10} y={22} width={def.width - 24} height={3} fill="#3b82f6" cornerRadius={1} />
      <Text x={def.width / 2 - 8} y={def.height - 18} text="TFT" fontSize={8} fill="#60a5fa" fontFamily="monospace" fontStyle="bold" />
    </Group>
  );
}
...
export function LoRaRenderer({ def }: RendererProps) {
  return (
    <Group>
      ...
      <Rect x={20} y={2} width={8} height={18} fill="#333" cornerRadius={1} />
      <Line points={[24, 2, 24, -6]} stroke="#666" strokeWidth={1} />
      <Text x={8} y={24} text="LoRa" fontSize={8} fill="#818cf8" fontFamily="monospace" fontStyle="bold" />
      <Text x={6} y={35} text="SX1278" fontSize={5} fill="#6366f1" fontFamily="monospace" />
    </Group>
  );
}
...
export function BluetoothRenderer({ def }: RendererProps) {
  return (
    <Group>
      ...
      <Text x={def.width / 2 - 6} y={8} text="🔵" fontSize={10} />
      <Text x={6} y={24} text="HC-05" fontSize={7} fill="#93c5fd" fontFamily="monospace" fontStyle="bold" />
    </Group>
  );
}

export function RGBStripRenderer({ def }: RendererProps) {
  return (
    <Group>
      <Rect x={0} y={0} width={def.width} height={def.height} fill="#1a1a2e" cornerRadius={3} stroke="#7c3aed" strokeWidth={1} />
      {Array.from({ length: 8 }, (_, i) => {
        const colors = ['#ef4444', '#f97316', '#eab308', '#22c55e', '#06b6d4', '#3b82f6', '#8b5cf6', '#ec4899'];
        return <Circle key={i} x={8 + i * 11} y={def.height / 2} radius={4} fill={colors[i]} shadowColor={colors[i]} shadowBlur={4} />;
      })}
    </Group>
  );
}
...
export function StepperRenderer({ def }: RendererProps) {
  return (
    <Group>
      <Circle x={def.width / 2} y={def.width / 2} radius={def.width / 2 - 2} fill="#374151" stroke="#059669" strokeWidth={1.5} />
      <Circle x={def.width / 2} y={def.width / 2} radius={def.width / 4} fill="#1f2937" stroke="#4b5563" strokeWidth={1} />
      <Line points={[def.width / 2, def.width / 2, def.width / 2, 6]} stroke="#ccc" strokeWidth={2} lineCap="round" />
      <Text x={6} y={def.width + 2} text="STEP" fontSize={6} fill="#6ee7b7" fontFamily="monospace" />
    </Group>
  );
}
...
export function RotaryEncoderRenderer({ def }: RendererProps) {
  return (
    <Group>
      ...
      <Circle x={def.width / 2} y={def.height / 2} radius={12} fill="#374151" stroke="#6b7280" strokeWidth={1} />
      <Circle x={def.width / 2} y={def.height / 2} radius={6} fill="#4b5563" />
      <Line points={[def.width / 2, def.height / 2, def.width / 2, def.height / 2 - 10]} stroke="#d1d5db" strokeWidth={1.5} lineCap="round" />
    </Group>
  );
}
...
export function JoystickRenderer({ def }: RendererProps) {
  return (
    <Group>
      ...
      <Circle x={def.width / 2} y={def.height / 2} radius={14} fill="#1f2937" stroke="#4b5563" strokeWidth={1} />
      <Circle x={def.width / 2} y={def.height / 2} radius={8} fill="#374151" shadowColor="#6b7280" shadowBlur={4} />
    </Group>
  );
}
